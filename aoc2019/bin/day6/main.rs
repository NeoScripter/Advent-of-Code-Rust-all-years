// I couldn't solve it myself, this is not my solution
...
impl Node {
    ...
    pub fn dist(&self, src: &str, target: &str) -> (u32, bool) {
        println!("Called dist on node: {}", self.name);
        if self.name == src || self.name == target {
            println!("Node is src or target");
            (0, true)
        } else {
            let dists = self
                .orbited_by
                .iter()
                .map(|x| {
                    println!("Recursing from node {} to node {}", self.name, x.borrow().name);
                    let result = x.borrow().dist(src, target);
                    println!("Result of recursion from node {} to node {}: {:?}", self.name, x.borrow().name, result);
                    result
                })
                .collect::<Vec<_>>();
            println!("Collected dists on node {}: {:?}", self.name, dists);
            let still_counting = dists.iter().filter(|(_, counting)| *counting).count();
            println!("Still counting on node {}: {}", self.name, still_counting);
            if still_counting == 2 {
